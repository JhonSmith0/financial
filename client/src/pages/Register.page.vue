        ...
        <form @submit="onSubmit" class="p-[4rem] w-full border items-start bg-[#f8f9fa] content-start ml-auto">
            ...
            <div class="grid gap-[2rem]">
                <fieldset :disabled="loading" class="grid gap-[1rem]">
                    <label class="font-poppins font-light text-[2.4rem]">Nome</label>
                    <div class="border-black rounded-[0.5rem] overflow-hidden border-opacity-10 border">
                        <input v-model="name" v-bind="nameAttrs" class="p-[2rem] w-full font-inter" type="text"
                            placeholder="John Smith" />
                    </div>
                    <div class="text-right">{{ errors.name }}</div>
                </fieldset>
                <fieldset :disabled="loading" class="grid gap-[1rem]">
                ...
                <fieldset :disabled="loading" class="grid gap-[1rem]">
                    <label class="font-poppins font-light text-[2.4rem]">Senha</label>
                    <div class="border-black rounded-[0.5rem] overflow-hidden border-opacity-10 border">
            ...
            <div class="flex mt-[6rem] gap-[1rem]">
                <RouterLink to="/login" class="w-full">
                    <Button :disabled="loading" type="submit" variant="ghost" class="w-full">
                        Logar
                    </Button>
                </RouterLink>
                <a class="w-full">
                    <Button :disabled="loading" variant="blue" class="w-full">
                        Registrar
                    </Button>
                </a>
            </div>
